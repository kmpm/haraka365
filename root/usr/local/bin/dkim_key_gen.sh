...
usage()
{
    echo "   usage: ${0} <bits 2048|4096> <basedir> <example.com>" 2>&1
    echo 2>&1
    exit 1
}
...
date '+%h%Y' | tr '[:upper:]' '[:lower:]' > "$DOMAINDIR/selector"
...
openssl genrsa -out "$DOMAINDIR/private" $BITS
chmod 0400 "$DOMAINDIR/private"
openssl rsa -in "$DOMAINDIR/private" -out "$DOMAINDIR/public" -pubout
